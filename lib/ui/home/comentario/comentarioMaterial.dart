...
class ComentarioState extends State<Comentario> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    starsBtn=[];
    for(int i=0; i<5; i++){
      starsBtn.add(starBtnPuntuacion(i+1, true),);
    }
  }
  ...
  Widget comentarioInfo(Map comentario){
    ...
    var bodyComentario=Container(
          ...
          return Container(
            ...
            child: Row(
              children: <Widget>[
                Expanded(child: puntuacion,),
                Expanded(flex: 2,child: bodyComentario,)
              ],
            ),
          );
  }
      ...
      onPressed: (){
        setState(() {
         ...
         starsBtn=[];
          for(int i=1; i<=5; i++){
            if(i<=punt){
              starsBtn.add(starBtnPuntuacion(i, true),);
            }else{
              starsBtn.add(starBtnPuntuacion(i, false),);
            }
          }
          print(punt);
        });
